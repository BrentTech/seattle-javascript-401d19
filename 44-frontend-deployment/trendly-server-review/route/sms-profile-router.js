...
//--------------------------------------------------
// vinicio - first third-party code
const { Router } = require('express');
const bodyParser = require('body-parser').urlencoded({extended:false});
const httpErrors = require('http-errors');
const superagent = require('superagent');
const MessagingResponse = require('twilio').twiml.MessagingResponse;
// vinicio - then, "our" code
//--------------------------------------------------
...
smsProfileRouter.post('/sms-profile', bodyParser, (request, response, next) => {
  ...
  const twiml = new MessagingResponse();// vinicio - bad name?
  ...
  // vinicio(as an annoying lead developer) - why aren't we using built-in functions?
  const isANumber = str => {
    return /\d+$/.test(str);
  };

  // vinicio - if you have indentation levels bigger than 5-6. It might be a 
  //           good idea to separate code into different modules/functions.
    ...
    smsProfile.findOne({phoneNumber})
      .then(foundProfile => {
        if (!foundProfile) {
          ...
          // vinicio - BAD NAMING CONVENTION: const api_url;
          ...
          return superagent.get(API_URL)
            .then(response => {
              ...
              // let meetupObject = response.body;
            })
            .then(meetupObject => {
              return superagent.get(API_GET_MEMBER_PROFILE)
                ...
                .then(newMeetupObject => {
                  ...
                  return new smsProfile ({
                    ...
                    .then(() => {
                      // vinicio - move the long string into a separate place in code or make it 
                      //           into a function that returns the string
                      twiml.message(`
...
'stop' - to opt out of text notifications`);
          ...
          // vinicio - move this string as well
          twiml.message(`
...
'stop' - to opt out of text notifications`);
    ...
    // vinicio - consider adding an unit of time
    const ONE_WEEK = 604800000;
    smsProfile.find({phoneNumber})
      .then(smsProfile => {
        if (smsProfile.length === 0) {
          // vinicio - consider moving this string somewhere else.;
          twiml.message(`
...
which can be found at https://www.meetup.com/account/`);
        ...
        // newbie vinicio - why are we using index 0? in both places?
        if (!smsProfile[0].meetups[0]) {
          // vinicio - adding i18n (Internationalization and localization) support
          //twiml.message(text.NO_MEETUPS);
          twiml.message(`No Meetups listed with your account`);
          response.writeHead(200, {'Content-Type': 'text/xml'});
          response.end(twiml.toString());
          // "Matt" - preventing future API call;
          return; // newbie vinicio - why are we returning undefined?
        }
          ...
          superagent.get(`https://api.meetup.com/${each}/events?key=${process.env.API_KEY}`)
            ...
            .then(eventsArray => {
              ...
              return filteredEvents.reduce((acc, each) => {
                // vinicio - consider extracting the regular expression to its
                //           own variable.
      ...
      .catch(next);
  // vinicio - naked strings are usually a "smell"
